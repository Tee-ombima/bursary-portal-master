...
from .form import add_scholarship_form,extra_documents_form,UpdateFieldWithCustomValueForm
from django.contrib import messages
from .models import application_table,session_table,scholarship
...
#
#obj = current_session.objects.get(id=0)
...
# session=timezone.now()
# try:
#     obj = current_session.objects.get(session_id=session_id)
#     nowdate = obj.session
# except current_session.DoesNotExist:
#     try:
#         # Try to retrieve the object again, in case another thread created it in the meantime
#         obj = current_session.objects.get(session_id=session_id)
#         nowdate = obj.session
#     except current_session.DoesNotExist:
#         # Create the table and the object
#         try:
#             current_session.objects.create(session_id=session_id)
#         except ProgrammingError:
#             # The table already exists, which is fine
#             pass
#         obj = current_session.objects.create(session_id=session_id)
#         nowdate = obj.session


    # create the object if it doesn't exist
    #obj = current_session.objects.create(id=3)
    #obj.save()
    #nowdate = obj.session
...
def year_value(x):
    if x == 'FE':
        ...
        return 0
def update_field_with_custom_input_view(request):
    if request.method == 'POST':
        queryset = application_table.objects.all()
        ...
        if form.is_valid():
            ...
            for obj in queryset:
                ...
                obj.save()
            messages.success(request, f"{len(queryset)} objects updated.")
...
def graphs():
    ...
    branchcount = {"RIFTVALLEY": 0, "EASTERN": 0, "NORTHEASTERN": 0, "CENTRAL": 0, "NAIROBI": 0, "COASTAL": 0, "WESTERN": 0, "NYANZA": 0}
    ...
    for app in appobjects:
        ...
        if appuser.profile.branch == 1:
            branchcount["EASTERN"] = branchcount["EASTERN"] + 1
        elif appuser.profile.branch == 2:
            branchcount["NORTHEASTERN"] = branchcount["NORTHEASTERN"] + 1
        elif appuser.profile.branch == 3:
            branchcount["RIFTVALLEY"] = branchcount["RIFTVALLEY"] + 1
        elif appuser.profile.branch == 4:
            branchcount["CENTRAL"] = branchcount["CENTRAL"] + 1
        elif appuser.profile.branch == 5:
            branchcount["COASTAL"] = branchcount["COASTAL"] + 1
        elif appuser.profile.branch == 6:
            branchcount["NAIROBI"] = branchcount["NAIROBI"] + 1
        elif appuser.profile.branch == 7:
            branchcount["NYANZA"] = branchcount["NYANZA"] + 1
        elif appuser.profile.branch == 8:
            branchcount["WESTERN"] = branchcount["WESTERN"] + 1
...
def show_application_form(request,x):
    form = extra_documents_form()
    if request.method == 'POST':
        form = extra_documents_form(request.POST)
        if form.is_valid():
            form.save()
...
def change_session(request):
    obj = session_table.objects.get(id=3)
    cs=obj.session
    cs1=cs+1
...
def change_status(request,x,y,z):
    ...
    y = application_table.get_my_integer_field_display
...
def show_selected_applicants(request,x):
    ...
    for apps in all_applications:
        Student = User.objects.get(id=apps.user_id)
        if Student.profile.branch == all_admins[4].branch and apps.status == 1 and apps.scholarship_id == int(x):
            branch_five.append((Student,apps))
    for apps in all_applications:
        Student = User.objects.get(id=apps.user_id)
        if Student.profile.branch == all_admins[5].branch and apps.status == 1 and apps.scholarship_id == int(x):
            branch_six.append((Student,apps))
    for apps in all_applications:
        Student = User.objects.get(id=apps.user_id)
        if Student.profile.branch == all_admins[6].branch and apps.status == 1 and apps.scholarship_id == int(x):
            branch_seven.append((Student,apps))
    for apps in all_applications:
        Student = User.objects.get(id=apps.user_id)
        if Student.profile.branch == all_admins[7].branch and apps.status == 1 and apps.scholarship_id == int(x):
            branch_eight.append((Student,apps))
    ...
    for admin in all_admins:
        if admin.branch == 1:
            all_admin_branches.append("EASTERN")
        elif admin.branch == 2:
            all_admin_branches.append("NORTHEASTERN")
        elif admin.branch == 3:
            all_admin_branches.append("RIFTVALLEY")
        elif admin.branch == 4:
            all_admin_branches.append("CENTRAL")
        elif admin.branch == 5:
            all_admin_branches.append("COASTAL")
        elif admin.branch == 6:
            all_admin_branches.append("NAIROBI")
        elif admin.branch == 7:
            all_admin_branches.append("NYANZA")
        else:
            all_admin_branches.append("WESTERN")
    ...
    context = {
        ...
        'branch_five':branch_five,
        'branch_six':branch_six,
        'branch_seven':branch_seven,
        'branch_eight':branch_eight,

        'branch0':all_admin_branches[0],
        'branch1': all_admin_branches[1],
        'branch2': all_admin_branches[2],
        'branch3': all_admin_branches[3],
        'branch4': all_admin_branches[4],
        'branch5': all_admin_branches[5],
        'branch6': all_admin_branches[6],
        'branch7': all_admin_branches[7],
        'branch8': all_admin_branches[8],
